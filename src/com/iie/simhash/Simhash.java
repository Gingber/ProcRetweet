...
public class Simhash {
	...
	public long simhash64(String doc) throws IOException {
		int bitLen = 64;
		int[] bits = new int[bitLen];
		List<String> tokens = wordSeg.tokens(doc);
		for (String t : tokens) {
			//long v = MurmurHash.hash64(t);
			long v = CityHash.cityHash64(t.getBytes(), 0, t.length());
	...
	public long simhash32(String doc) throws IOException {
		int bitLen = 32;
		int[] bits = new int[bitLen];
		List<String> tokens = wordSeg.tokens(doc);
		for (String t : tokens) {
